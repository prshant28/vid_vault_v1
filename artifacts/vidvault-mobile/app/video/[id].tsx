...
function AiOutputPanel({ output, tool, videoTitle, onClose, onRegenerate }: {
  ...
  const handleShare = async () => {
    try {
      const shareContent = [
        `📺 ${videoTitle || "Video"} — ${tool.label}`,
        "─".repeat(40),
        output.content,
        "",
        "Generated by VidVault AI",
      ].join("\n");
      await Share.share({ message: shareContent, title: `${tool.label} — ${videoTitle || "Video"}` });
      ...
      <View style={styles.outputPanelHeader}>
        <View style={{ flexDirection: "row", alignItems: "center", gap: 10 }}>
          ...
          <View style={{ flex: 1 }}>
            ...
            {videoTitle ? (
              <Text style={{ fontSize: 10, color: "rgba(255,255,255,0.35)", fontFamily: "JetBrainsMono", marginTop: 1 }} numberOfLines={1}>{videoTitle}</Text>
            ) : null}
          </View>
        </View>
        <View style={{ flexDirection: "row", gap: 8 }}>
          <TouchableOpacity onPress={handleShare} style={styles.regenBtn} activeOpacity={0.8} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}>
            <Feather name="share-2" size={13} color={CYAN} />
          </TouchableOpacity>
