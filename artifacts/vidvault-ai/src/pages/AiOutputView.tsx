...
function mdToHtml(t: string): string {
  try { return marked.parse(t) as string; } catch { return `<p>${t}</p>`; }
}
...
export default function AiOutputView() {
  ...
  const handleRegenerate = async () => {
    ...
    try {
      ...
      await refetch();
      toast({ title: "Regenerated!", description: `${meta.label} has been refreshed.` });
    } catch (err: any) {
        ...
        {isLoading ? (
          ...
          >
            ...
            <div
              ...
              style={{ background: "var(--vv-surface)", border: "1px solid var(--vv-card-border)" }}
            >
              <span className="text-[9px] font-mono-ui uppercase tracking-wider text-muted-foreground mr-2">Export As:</span>
